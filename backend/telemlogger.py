...
class TelemetryLogger(object):
    ...
    def resume_file(self, filepath: str, callsign: str):
        """
        Registriraj postojeću log datoteku za callsign tako da novi paketi
        nastave u istu datoteku umjesto da otvaraju novu.
        Poziva se nakon učitavanja starog loga (resume flight).
        """
        import os.path as osp
        if not osp.exists(filepath):
            logging.warning(f"Telemetry Logger - Resume file not found: {filepath}")
            return False

        if filepath.endswith('.csv'):
            self.csv_filenames[callsign] = filepath
            logging.info(f"Telemetry Logger - Resumed CSV log for {callsign}: {filepath}")
        elif filepath.endswith('.json'):
            self.json_filenames[callsign] = filepath
            logging.info(f"Telemetry Logger - Resumed JSON log for {callsign}: {filepath}")
        else:
            logging.warning(f"Telemetry Logger - Unknown file extension for resume: {filepath}")
            return False
        return True
